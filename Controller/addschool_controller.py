...
def save_image_to_project(image_path: str, destination_folder: str, new_name: str):
    ...
    if not isinstance(image_path, str) or not os.path.exists(image_path):
        messagebox.showerror("Lỗi", "Đường dẫn ảnh không tồn tại")
        return
...
class AddSchoolController:
    ...
    def add_university(self, name, location, region, focus, country_fee,
                       global_fee, SAT, TOEFL, description, idUser, image):
    ...
    def home(self):
        ...
        for widget in self.root.winfo_children():
            widget.destroy()
        regis_view = NewHomeAdminView(self.root, None)  # Ban đầu không truyền Controller
        regis_controller = NewHomeAdminController(self.root, regis_view)
        regis_view.controller = regis_controller
        self.view = regis_view
        self.display_view()
